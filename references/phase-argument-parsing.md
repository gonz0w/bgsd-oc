...
```bash
PHASE_INFO=$(node /home/cam/.config/opencode/get-shit-done/bin/gsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node /home/cam/.config/opencode/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node /home/cam/.config/opencode/get-shit-done/bin/gsd-tools.cjs find-phase "${PHASE}" | jq -r '.directory')
```
