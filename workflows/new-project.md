...
Create roadmap: derive phases, map every v1 requirement, 2-5 success criteria per phase, 100% coverage. Validate requirement-to-phase traceability first without relying on phase summaries or other post-execution delivery artifacts. Write ROADMAP.md, STATE.md, update REQUIREMENTS.md traceability. Initialize `STATE.md` Current Position with numeric execution-ready phase/plan values that match the first phase instead of `Not started` or `TBD`. Return ROADMAP CREATED.
