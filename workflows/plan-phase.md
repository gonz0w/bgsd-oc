...
When authoring `must_haves.artifacts` metadata, choose implementation-stable evidence strings (exported function names, field names, exact shipped guidance text) instead of prose summaries or generic command labels. Use verifier-stable tokens only: put one expected marker per entry, not comma-joined prose or mixed multi-token `contains` strings.
When a plan body needs to mention commands for comparison, legacy context, or invalid examples, write them as explicit reference-only text with placeholders like `[phase]` or plain-language aliases. Do not embed malformed or runnable-looking slash-command examples in must_haves, behavior prose, or approval-time metadata unless execution guidance is intentional.
When writing task `<verify>` commands, prefer simple `node --test`, `npm`, `rg`, or direct file-check commands. Avoid `node -e`, shell substitutions, or other inline scripting unless the extra complexity is essential to proving the plan truth. Each task `<verify>` should prove only that task's delta; reserve the plan-level `<verification>` block for the aggregate final proof.
For validator-change or command-validator-expansion slices, keep approval-time verification on build plus targeted tests against current shipped behavior. Do not rely on newly accepted command examples in the plan's approval-time `<verification>` commands before the validator change lands.
When TDD coverage touches shared docs or oversized test files with known baseline failures or hangs, pair any broad file-level gate with one isolated focused smoke command so RED/GREEN proof stays attributable to the owned slice.
