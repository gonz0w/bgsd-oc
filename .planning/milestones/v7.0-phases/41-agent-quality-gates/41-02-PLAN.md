...
<task type="auto">
  ...
  <action>
  Create `references/reviewer-agent.md` — the behavioral reference for the gsd-reviewer agent pattern. This is NOT an AI coding assistant agent definition file; it's a reference document loaded by the execute-plan workflow when performing post-execution review.
