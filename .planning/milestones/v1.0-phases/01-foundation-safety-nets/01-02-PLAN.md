...
```bash
node bin/gsd-tools.cjs validate-config --raw
```
...
Run `node bin/gsd-tools.cjs validate-config --raw 2>/dev/null` — should return valid JSON with fewer unknown key warnings.
Run `node bin/gsd-tools.cjs init progress --raw 2>/dev/null | python3 -c "import json,sys; d=json.load(sys.stdin); print('OK')"` — config loading still works for the project.
