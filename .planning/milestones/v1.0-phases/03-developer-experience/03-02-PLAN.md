...
Purpose: 11 feature commands (session-diff, context-budget, test-run, etc.) exist in gsd-tools.cjs but have no slash command wrappers — they're invisible to users. The parallel execution visualization helps users understand which plans can run simultaneously.
...
1. **cmd-session-diff.md** — `/gsd-session-diff` — Shows git commits since last activity
2. **cmd-context-budget.md** — `/gsd-context-budget <plan-path>` — Estimates token usage for a plan
3. **cmd-test-run.md** — `/gsd-test-run` — Parses test output with pass/fail gating
4. **cmd-search-decisions.md** — `/gsd-search-decisions <query>` — Searches STATE.md + archives for decisions
5. **cmd-validate-deps.md** — `/gsd-validate-deps <phase>` — Validates phase dependency graph
6. **cmd-search-lessons.md** — `/gsd-search-lessons <query>` — Searches completed phase lessons
7. **cmd-codebase-impact.md** — `/gsd-codebase-impact <files...>` — Shows module dependencies
8. **cmd-rollback-info.md** — `/gsd-rollback-info <plan-id>` — Shows commits + revert command
9. **cmd-velocity.md** — `/gsd-velocity` — Plans/day + completion forecast
10. **cmd-trace-requirement.md** — `/gsd-trace-requirement <req-id>` — Full requirement-to-file trace
11. **cmd-validate-config.md** — `/gsd-validate-config` — Config schema validation
...
Each should accept appropriate arguments (using `$ARGUMENTS` or `$1` patterns from the command system) and display results in a human-readable format with appropriate formatting.
  ...
  <verify>
Run: `ls workflows/cmd-*.md | wc -l` — should be 11.
Run: `grep -l 'gsd-tools.cjs' workflows/cmd-*.md | wc -l` — all 11 should reference gsd-tools.cjs.
  </verify>
...
Wave 1 (parallel): ┌─ 03-01-PLAN.md (--help + config migrate)
...
3. Show parallel plans in the same wave with branching lines (┌─ ├─ └─)
...
This is a presentation enhancement — it reads data that's already computed, not new computation.
  </action>
  <verify>
