...
Add routing in `main()` switch: `case 'env':` with subcommand `scan` → `cmdEnvScan(cwd, args, raw)`.
  ...
  <verify>
Run: `node bin/gsd-tools.cjs env scan --raw` from the gsd-opencode project root.
Should produce valid JSON with at least `languages` containing `node` (from package.json — wait, gsd-opencode has no package.json, but it has the plugin's own structure).

Better test: Run against the gsd-opencode directory itself — should detect at minimum JavaScript from bin/gsd-tools.cjs presence, and possibly Makefile/Justfile if present. Or test against a known polyglot project directory.
