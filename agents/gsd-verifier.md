...
```bash
PHASE_DATA=$(node $GSD_HOME/bin/gsd-tools.cjs plan:roadmap get-phase "$PHASE_NUM" --raw)
```
...
SUMMARY_FILES=$(node $GSD_HOME/bin/gsd-tools.cjs util:summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node $GSD_HOME/bin/gsd-tools.cjs verify:verify commits $COMMIT_HASHES)
fi
